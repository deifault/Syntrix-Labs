...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (subcommand === 'add') {
      ...
      db.prepare('INSERT INTO vouches (user_id, voucher_id, rating, comment, created_at) VALUES (?, ?, ?, ?, ?)').run(
        user.id,
        interaction.user.id,
        rating,
        comment,
      ...
      const embed = new EmbedBuilder()
        ...
        .setTitle(`${config.emojis.vouch} New Vouch Added`)
      ...
      const vouchChannel = interaction.guild?.channels.cache.get(config.channels.vouchChannel);
      if (vouchChannel && vouchChannel.isTextBased()) {
        await vouchChannel.send({ embeds: [embed] });
      }

      await interaction.reply({ content: `${config.emojis.success} Vouch added successfully!`, ephemeral: true });
    }

    if (subcommand === 'check') {
      const user = interaction.options.getUser('user', true);
      
      const vouches = db.prepare('SELECT * FROM vouches WHERE user_id = ? ORDER BY created_at DESC').all(user.id) as any[];
      ...
      const embed = new EmbedBuilder()
        ...
        .setTitle(`${config.emojis.vouch} Vouches for ${user.username}`)
        .setDescription(vouchList)
        .addFields(
          { name: 'Total Vouches', value: vouches.length.toString(), inline: true },
