  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (subcommand === 'setup') {
      const embed = new EmbedBuilder()
        ...
        .setTitle(`${config.emojis.order} Order System`)
        ...
        .addFields(
          { name: 'Processing Time', value: '24-48 hours', inline: true },
          { name: 'Payment', value: 'After confirmation', inline: true }
        )
        .setTimestamp()
    ...
    if (subcommand === 'complete') {
      ...
      const embed = new EmbedBuilder()
        ...
        .setTitle(`${config.emojis.success} Order Completed`)
        ...
        .addFields(
          ...
          { name: 'Total', value: `$${order.total_price.toFixed(2)}`, inline: true }
        )
        .setTimestamp();
    ...
    if (subcommand === 'cancel') {
      ...
      const embed = new EmbedBuilder()
        ...
        .setTitle(`${config.emojis.error} Order Cancelled`)
    ...
    if (subcommand === 'list') {
      ...
      const embed = new EmbedBuilder()
        .setColor(config.colors.info)
        .setTitle(`${config.emojis.order} Orders List`)
